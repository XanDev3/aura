...
function EntryRow({ entry }: { entry: DecisionLogEntry }) {
  ...
  return (
    <div className="flex items-start gap-3 py-3 border-b border-[#1e2a4a] last:border-0">
      <div className="flex-none text-xs text-slate-600 w-14 text-right pt-0.5">
        #{entry.tick}
      </div>

      <div className={`flex-none text-xs font-mono font-bold w-16 ${actionStyle.color}`}>
        {actionStyle.label}
      </div>
  ...
  return (
    <div className="bg-[#0f1628] border border-[#1e2a4a] rounded-xl p-5">
      ...
      {state.zgStorageRoots.length > 0 && (
        <div className="mt-3 pt-3 border-t border-[#1e2a4a] text-xs text-slate-600">
